...
(function () {
  ...
  /* removed: var _$d3Node_32 = require('d3'); */; // Broadest functions
  ...
  var mainColor = '#29B6A3';
  var lighterColor = '#4DC1B4';
  var negativeColor = '#E64060';
  var colorArr = ['#7F4893', //1: "Education"
  ...
  var tooltip = _$d3Node_32.select('.tooltip'); // Overall functions
  ...
  _$d3Node_32.selection.prototype.dedupeLabels = function () {
    if (showingLabels) {
      ...
      allDedupeLabels.each(function (d, i) {
        ...
        allDedupeLabels.filter(function (k, j) {
          return j > i;
        }).each(function (d) {
  ...
  var getOverlapFromTwoExtents = function getOverlapFromTwoExtents(l, r) {
  ...
  var getCircleSize = function getCircleSize(d) {
    return d.group_average == 0 ? circleSize : circleSize * 1.5;
  }; // Process average data to append ID


  var processedAvg = data.filter(function (d) {
    return d.group_average == 1;
  }).map(function (d, i) {
    d.id = 'avg-id-' + i;
    return d;
  ...
  function build(data, averageData) {
    // Data vars
    var programColumn = 'program';
    var groupColumn = 'group';
    var labelColumn = 'small_label_name';
    var longLabelColumn = 'long_description';
    var parentCategoryColumn = 'group'; // Parent category

    var categoryColumn = 'prog_type'; // Program category

    var baselineColumn = 'baseline';
    var restrictedColumn = 'restricted';
    var extendedColumn = 'extended';
    var idColumn = 'id';
    var mvpfColumn = 'mvpf';
    var ageColumn = 'age_benef';
    var yearColumn = 'year_implementation';
    var costColumn = 'c_on_pc';
    var willingnessColumn = 'w_on_pc';
    var citationsColumn = 'paper_cite_keys';
    var avgLabel = '_avg';
    var upperCI = '_u';
    var lowerCI = '_l'; // Assumptions

    var assumptionRobustness = '_rob';
    var assumptionLowerBound = '_lbwtp';
    var assumptionPublicationBias = '_pbc';
    var assumptionSel = ''; // Samples

    var sampleBaseline = '';
    var sampleRestricted = '_res';
    var sampleExtended = '_ext';
    var sampleSel = '';
    var assumptionPublicationBiasRobustness = assumptionPublicationBias + assumptionRobustness;
    var assumptions = [assumptionRobustness, assumptionLowerBound, assumptionPublicationBias, sampleRestricted, sampleExtended];
    var ySelExt = ''; // Arrays from data

    var categories = _$d3Node_32.nest().key(function (d) {
      return d[categoryColumn];
    }).sortValues(function (a, b) {
      return a - b;
    }).entries(data).map(function (d) {
      return d.key;
    });
    var parentCategories = _$d3Node_32.nest().key(function (d) {
      return d[parentCategoryColumn];
    }).entries(data).map(function (d) {
      return d.key;
    }).filter(function (d) {
      return d !== '';
    }).sort();
    var programNames = _$d3Node_32.nest().key(function (d) {
      return d[programColumn];
    }).entries(data).map(function (d) {
      return d.key;
    });
    var programShortNames = _$d3Node_32.nest().key(function (d) {
      return d[labelColumn];
    }).entries(data).map(function (d) {
      return d.key;
    });
    var programLongNames = _$d3Node_32.nest().key(function (d) {
      return d[longLabelColumn];
    }).entries(data).map(function (d) {
      return d.key;
    });
    var programLabelsHashed = _$d3Node_32.map(data, function (d) {
      return d[longLabelColumn];
    });
    var upfrontHighlightsKeys = Object.keys(upfrontHighlights);
    var upfrontHighlightsHashed = _$d3Node_32.map(upfrontHighlights, function (d) {
      return d;
    });
    var labelNames = _$d3Node_32.map(labelMap, function (d) {
      return d.variable;
    }); // Selections

    var categorySelections = categories;
    var programHighlight = null;
    var ySelections = [mvpfColumn, costColumn, willingnessColumn];
    var xSelections = [ageColumn, yearColumn];
    var tableValues = [programColumn, costColumn, willingnessColumn, mvpfColumn, citationsColumn]; // Add matching value for averages on each program

    processedData.map(function (d, i) {
      processedAvg.forEach(function (k) {
    ...
    _$d3Node_32.select('#dropdown-domains').selectAll('div.category-item').data(categories).enter().append('div').attr('class', function (d) {
      ...
      return d + ' <a href="#only" class="only">Only</a>';
    }); // Make dropdown selector

    _$d3Node_32.select('#dropdown-programs').selectAll('option').data(programLongNames).enter().append('option').text(function (d) {
      return d;
    }); // Dropdown
    // Change location
    ...
    $('.hover-category').hover(function () {
      ...
      circles.transition().duration(defaultAnimationTime).attr('fill', function (d) {
        return d[groupColumn] == $(_this).data('group') ? colorScale(d[parentCategoryColumn]) : '#eee';
      });
      dataGroup.filter(function (d) {
        return d[groupColumn] == $(_this).data('group');
      }).moveToFront();
    ...
    $('#controls #lbwtp-options').change(function () {
      ...
      setTimeout(function () {
        assumptionSel = $(_this9).data('value') + $(_this9).val();
        updateViz();
      });
    }); // Controls
    ...
    $('#dropdown-domains .checkbox').not('.only').on('mouseup', function (e) {
      ...
      e.stopPropagation();
      setTimeout(function () {
        if ($(_this10).hasClass('show-hide-all')) {
          if ($(_this10).hasClass('selected')) {
            $('#dropdown-domains .checkbox').addClass('selected');
            categorySelections = categories;
          } else {
          ...
          if (allSelected) {
            categorySelections = [$(_this10).data('domain')];
            $('#dropdown-domains .checkbox.selected').removeClass('selected');
            $(_this10).addClass('selected');
          } else {
            if ($(_this10).hasClass('selected')) {
              // Add to categories
              categorySelections.push($(_this10).data('domain'));
            } else {
              // Remove from categories
              categorySelections.splice(categorySelections.indexOf($(_this10).data('category')), 1);
            }
          }
        }
    ...
    $('#dropdown-domains .checkbox .only').on('mouseup', function (e) {
      ...
      e.stopPropagation();
      setTimeout(function () {
        categorySelections = [$(_this11).parent().data('domain')];
        $('#dropdown-domains .checkbox').removeClass('selected');
        $(_this11).parent().addClass('selected'); // Update checkboxes
    ...
    $('#controls #assumptions.radio-group .radio').on('mouseup', function () {
      ...
      setTimeout(function () {
        assumptionSel = $(_this12).data('value'); // If this is the former LBTWP item, we need to append the matching dropdown

        if ($(_this12).hasClass('lbtwp')) {
          assumptionSel += $('#lbwtp-options').val();
        }

        if ($(_this12).hasClass('rob_dropdown')) {
    ...
    var x = _$d3Node_32.scaleLinear().range([0, width]).domain(_$d3Node_32.extent(data, function (d) {
      return d[xSel];
    }));
    var ny = _$d3Node_32.scaleLinear().range([height, 0]).domain(_$d3Node_32.extent(data, function (d) {
    ...
    var simulation = _$d3Node_32.forceSimulation(processedData.filter(function (d) {
      return d[baselineColumn] == 1;
    })).force('x', _$d3Node_32.forceX(function (d) {
      return x(d[xSel]);
    }).strength(1)).force('y', _$d3Node_32.forceY(height / 2)).force('collide', _$d3Node_32.forceCollide().radius(function (d) {
      return circleSize;
    })).stop();
    ...
    var origData = processedData.filter(function (d) {
      return d[baselineColumn] == 0;
    }).map(function (d) {
      d.y = height / 2;
      return d;
    });
    ...
    var yAxisTicks = ny.ticks().filter(function (tick) {
    ...
    var yAxisLine = yAxisGroup.selectAll('line').attr('x1', 0).attr('x2', width).filter(function (d) {
      return d === 0;
    }).style('stroke', '#bfbfbf'); // Data viz

    var dataGroup = svg.selectAll('g.datagroup').data(data).enter().append('g').attr('class', 'datagroup').attr('transform', function (d) {
      return forced ? "translate(" + x(d[xSel]) + ", " + d.y + ")" : "translate(" + x(d[xSel]) + ", " + y(d[ySel]) + ")";
    }).style('opacity', 1);
    var circles = dataGroup.append('circle').attr('r', function (d) {
    ...
    programCIGroup.attr('transform', function (d) {
      var xAvg = x(d[xSel]) ? x(d[xSel]) : 0;
      var yAvg = 0;
      return "translate(" + xAvg + ", " + yAvg + ")";
    }).style('opacity', function (d) {
      return showingProgramCI ? 1 : 0;
    });
    var programConfidenceInterval = programCIGroup.append('line').attr('x1', function (d) {
      return 0;
    }).attr('x2', function (d) {
      return 0;
    }).attr('y1', function (d) {
      return y(d[ySel + lowerCI]);
    }).attr('y2', function (d) {
      return y(d[ySel + upperCI]);
    }).attr('stroke-width', 2).style('opacity', .5).attr('stroke', function (d) {
      return colorScale(d[parentCategoryColumn]);
    });
    var averageGroup = svg.selectAll('g.avg').data(averageData).enter().append('g').attr('class', 'avg').style('opacity', function (d) {
      return showingAverages ? 1 : 0;
    });
    averageGroup.attr('transform', function (d) {
      return "translate(" + x(d[xSel]) + ", " + y(d[ySel]) + ")";
    });
    var averageCircles = averageGroup.append('circle').attr('r', circleSize * 1.5).attr('fill', function (d) {
      return colorScale(d[parentCategoryColumn]);
    }).attr('stroke-width', 0).attr('stroke', '#333');
    var confidenceIntervalGroup = svg.selectAll('g.avg-ci').data(averageData).enter().append('g').attr('class', 'avg-ci');
    confidenceIntervalGroup.attr('transform', function (d) {
      return "translate(" + x(d[xSel]) + ", " + y(d[ySel]) + ")";
    }).style('opacity', function (d) {
      return showingCI ? 1 : 0;
    });
    var confidenceInterval = confidenceIntervalGroup.append('line').attr('x1', function (d) {
      return 0;
    }).attr('x2', function (d) {
      return 0;
    }).attr('y1', function (d) {
      return y(d[ySel + lowerCI]) ? y(d[ySel + lowerCI]) - y(d[ySel]) : 0;
    }).attr('y2', function (d) {
      return y(d[ySel + upperCI]) ? y(d[ySel + upperCI]) - y(d[ySel]) : 0;
    }).attr('stroke-width', 3).style('opacity', .5).attr('stroke', function (d) {
      return colorScale(d[parentCategoryColumn]);
    }); // Create voronoi
    ...
    var voronoiPaths = voronoi.selectAll(".voronoi-cell").data(voronoiData).enter().append("path").attr("d", function (d) {
      for (var i in d) {
        if (i != 'data') {
          d[i] = [d[i][0], d[i][1]];
        }
    ...
    }).style('stroke', '#333').style('stroke-width', showVoronoi ? 1 : 0).attr('class', 'voronoi-cell').style("fill", function (d) {
      return 'transparent';
    }).on('mousemove', showTooltip).on('mouseout', hideTooltip); // // Append labels to voronoi

    var voronoiLabels = svg.selectAll('.vlabel').data(preData).enter().append('text').attr('x', function (d) {
      return x(d[xSel]);
    }).attr('y', function (d) {
      return forced ? d.y : y(d[ySel]);
    }).attr('class', 'vlabel dedupe').text(function (d) {
      return d[labelColumn] ? d[labelColumn] : d[categoryColumn];
    }).style('opacity', 0); // Tooltips
    ...
    function showTooltip(d, fromSelection, showLabel, forcedLabel) {
      if (typeof d === undefined || !d || forced && !forcedLabel || killTooltipHover) {
        ...
        if (showingAverages || showingCI) {
          var matchingDot = averageGroup.filter(function (k) {
            return k[idColumn] == id;
          });

          if (matchingDot.size() == 0) {
            var matchingDot = dataGroup.filter(function (k) {
              return k[idColumn] == id;
            });
          }
        } else {
    ...
    function hideTooltip(d) {
      if (!killTooltipHover) {
        dataGroup.selectAll('circle').attr('stroke-width', function (k) {
          return 0;
        });
        averageGroup.selectAll('circle').attr('stroke-width', function (k) {
          return 0;
        });
        $('.tooltip').css('opacity', 0);
      }
    ...
    function appendAssumptions() {
      ...
      assumptions.forEach(function (d) {
        ySel = ySel.split(d)[0];
      });
    ...
    function highlightDot(dot, e, n, forceIt) {
      ...
      _$d3Node_32.select(dot).append('circle').attr('r', 5).attr('fill', function (d) {
        return colorScale(d[parentCategoryColumn]);
      }).style('opacity', .25).transition().duration(defaultAnimationTime).attr('r', 25).style('opacity', 0).on('end', function (d) {
    ...
    function updateViz() {
      ...
      if (redoAxesBy != 'auto') {
        ...
        if (showingAverages) {
          ...
          var averageXExtent = _$d3Node_32.extent(averageData, function (d) {
            return d[xSel];
          });
          var averageYExtent = _$d3Node_32.extent(averageData, function (d) {
            return yConvertN(d[ySelActive]);
          });
        ...
        if (showingCI) {
          ...
          ySelActive = averageYSel !== null ? averageYSel : ySel;
          var CIXExtent = _$d3Node_32.extent(averageData, function (d) {
            return d[xSel];
          });
          var CIYExtent = [_$d3Node_32.min(averageData, function (d) {
            return yConvertN(d[ySelActive + lowerCI]);
          }), _$d3Node_32.max(averageData, function (d) {
            return yConvertN(d[ySelActive + upperCI]);
          })];
        ...
        if (showingProgramCI) {
          ...
          var CIXExtent = _$d3Node_32.extent(data, function (d) {
            return d[xSel];
          });
          var CIYExtent = [_$d3Node_32.min(data, function (d) {
            return yConvertN(d[ySel + avgLabel + lowerCI]);
          }), _$d3Node_32.max(data, function (d) {
            return yConvertN(d[ySel + avgLabel + upperCI]);
          })];
      ...
      circles.transition().duration(defaultAnimationTime).attr('stroke-width', function (d) {
        return d[programColumn] == programHighlight ? 2 : 0;
      }).style('opacity', function (d) {
        return d.opacity;
      }).attr('fill', function (d) {
      ...
      programCIGroup.transition().duration(defaultAnimationTime).attr('transform', function (d) {
        ...
        return showingProgramCI && d.thisIn ? 1 : 0;
      });
      programConfidenceInterval.transition().duration(defaultAnimationTime).attr('y1', function (d) {
        return y(d[ySel + lowerCI]);
      }).attr('y2', function (d) {
        return y(d[ySel + upperCI]);
      });
      ...
      averageCircles.each(function (d) {
        var thisIn = true;
      ...
      confidenceInterval.transition().duration(defaultAnimationTime).attr('x1', function (d) {
        return 0;
      }).attr('x2', function (d) {
        return 0;
      }).attr('y1', function (d) {
        return y(d[averageYSelActive + lowerCI]) ? y(d[averageYSelActive + lowerCI]) : 0;
      }).attr('y2', function (d) {
      ...
      if (check != null && highlightDots) {
        if (showingPrograms && !showingAverages) {
          dataGroup.filter(function (d) {
            return d.thisIn;
          }).append('circle').attr('r', circleSize).attr('fill', function (d) {
            return colorScale(d[parentCategoryColumn]);
          }).style('opacity', .25).transition().duration(defaultAnimationTime).attr('r', circleSize * 6).style('opacity', 0).on('end', function () {
            highlightDot(this.parentNode, this, 0);
          });
        }

        if (showingAverages) {
          averageGroup.filter(function (d) {
            return d.thisIn;
          }).append('circle').attr('r', circleSize).attr('fill', function (d) {
            return colorScale(d[parentCategoryColumn]);
          }).style('opacity', .25).transition().duration(defaultAnimationTime).attr('r', circleSize * 6).style('opacity', 0).on('end', function () {
      ...
      yAxisTicks = ny.ticks().filter(function (tick) {
        return Number.isInteger(tick);
      });
      ...
      if (xSel == yearColumn) {
        ...
        xAxisGroup.selectAll('.tick text').text(function (d) {
          return parseInt(d);
        });
      ...
      yAxisGroup.selectAll('.tick').filter(function (d) {
        return d === 0;
      }).select('line').style('stroke', '#bfbfbf'); // Ensure MVPF includes special formatting
      ...
      if (ySel.match(mvpfColumn) !== null) {
        ...
        var top = yAxisGroup.selectAll('.tick').filter(function (d) {
          return d == 6;
        });
      ...
      voronoiPaths.enter().append('path').attr('d', 'M 0,0, L 1,1 Z').style('stroke', '#333').style('stroke-width', showVoronoi ? 1 : 0).attr('class', 'voronoi-cell').style("fill", function (d) {
        return 'transparent';
      }).on('mousemove', showTooltip).on('mouseout', hideTooltip);
      voronoiPaths = voronoi.selectAll('.voronoi-cell');
      voronoiPaths.attr("d", function (d) {
        for (var i in d) {
          if (i != 'data') {
            d[i] = [d[i][0], d[i][1]];
          }
      ...
      voronoiLabels.enter().append('text').attr('x', function (d) {
        return x(d[xSel]);
      }).attr('y', function (d) {
      ...
      .each(function (d) {
        ...
        var matchingVoronoi = voronoiPaths.filter(function (k) {
        ...
        var angle = Math.round(Math.atan2(cy - dy, cx - dx) / Math.PI * 2);
        var orientSel = angle === 0 ? 'right' : angle === -1 ? 'top' : angle === 1 ? 'bottom' : 'left'; // Determine whether to show or hide dependnig on whether label "fits" voronoi
    ...
    var updateSlide = function updateSlide(type) {
      // Defaults
      categorySelections = categories;
      applyCheckTo = null;
      check = null;
  ...
  var makeBarChart = function makeBarChart(bcDataArr, bc2DataArr) {
    // Chart vars
    ...
    var bcMaxYVal2 = 160000; //d3.max(bc2DataRaw, d => d.val)*2

    var bcX2 = _$d3Node_32.scaleLinear().domain([0, 6]).range([0, bcWidth]);
    var bcH2 = _$d3Node_32.scaleLinear().domain([0, bcMaxYVal2]).range([0, bcHeight / 2]);
    var bcY2 = _$d3Node_32.scaleLinear().domain([0, bcMaxYVal2]).range([bcHeight / 2, 0]); // Scales

    function makeByChartByData(bcSvg, bcData, bcXThis, bcYThis, bcHThis) {
      var barMargin = 5;
      var barGroups = bcSvg.selectAll('g.bars').data(bcData).enter().append('g').attr('class', 'bars bc-g').attr('transform', function (d, i) {
        var y = bcYThis(d.runningTotal);
        return "translate(" + bcXThis(i) + ", " + y + ")";
      });
      var barLabels = barGroups.append('text').text(function (d) {
        var neg = d.val < 0 ? '-' : '';
        return neg + '$' + numberWithCommas(Math.round(Math.abs(Number(d.val))).toLocaleString('en'));
      }).attr('y', function (d) {
        return d.val > 0 ? -5 : bcHThis(Math.abs(d.val)) + 15;
      }).attr('x', bcXThis(1) / 2).attr('class', 'label label-num bc-text').style('opacity', 0);
      var barNames = barGroups.append('g').attr('class', 'label-name-group').style('opacity', 0).attr('transform', function (d) {
        return "translate(" + bcXThis(1) / 2 + ", " + (d.val > 0 ? bcHThis(Math.abs(d.val)) + 15 : -5 - 12 * (d.labelArr.length - 1)) + ")";
      }).selectAll('.label-name').data(function (d) {
        return d.labelArr;
      }).enter().append('text').text(function (d) {
        return d;
      }).attr('class', 'label label-name bc-text').attr('y', function (d, i) {
        return i * 12;
      });
      var bars = barGroups.append('rect').attr('class', 'bc-rect').attr('width', bcXThis(1) - barMargin * 2).attr('height', 0).attr('x', barMargin).attr('transform', function (d) {
        return d.val < 0 ? 'translate(0, 0)' : "translate(0, " + bcHThis(d.val) + ")";
      }).attr('fill', function (d, i) {
        return d.val > 0 ? mainColor : '#0074A2';
      });
    } // Update slide
    ...
    var updateBCSlide = function updateBCSlide(type, title) {
      type = parseInt(type);
      ...
      if (type > 5 && type < 13) {
        ...
        bcSvg.selectAll('.bc-rect').transition().duration(1000).attr('height', function (d, i) {
          return i <= type ? bcH(Math.abs(d.val)) : 0;
        }).attr('transform', function (d, i) {
          return i <= type ? 'translate(0, 0)' : d.val < 0 ? 'translate(0, 0)' : "translate(0, " + bcH(d.val) + ")";
        });
        bcSvg.selectAll('.label-num.bc-text').transition().duration(1000).style('opacity', function (d, i) {
          return i <= type ? 1 : 0;
        });
        bcSvg.selectAll('.label-name-group').transition().duration(1000).style('opacity', function (d, i) {
          return i <= type ? 1 : 0;
        });
      } else {
        ...
        bc2Svg.selectAll('.bc-rect').transition().duration(1000).attr('height', function (d, i) {
          return i <= type ? bcH2(Math.abs(d.val)) : 0;
        }).attr('transform', function (d, i) {
          return i <= type ? 'translate(0, 0)' : d.val < 0 ? 'translate(0, 0)' : "translate(0, " + bcH2(d.val) + ")";
        });
        bc2Svg.selectAll('.label-num.bc-text').transition().duration(1000).style('opacity', function (d, i) {
          return i <= type ? 1 : 0;
        });
        bc2Svg.selectAll('.label-name-group').transition().duration(1000).style('opacity', function (d, i) {
          return i <= type ? 1 : 0;
        });
      }
    ...
    function makeLineChart(data) {
      ...
      var lineGenerator = _$d3Node_32.line().x(function (d) {
        return x(d.values[0][lineXSel]);
      }).y(function (d) {
        return y(d.values[0][lineYSel]);
      });
      var secondLineGenerator = _$d3Node_32.line().x(function (d) {
        return x(d.values[0][lineXSel]);
      }).y(function (d) {
        return secondY(d.values[0][lineYSel]);
      });
      ...
      yAxisGroup.selectAll('text').text(function (d, i) {
        return d == 50000 ? '$' + d / 1000 + 'K' : d / 1000;
      });
      secondYAxisGroup.selectAll('text').text(function (d, i) {
      ...
      function addLine(sel, linegen) {
        lineYSel = sel;
        lineGroup.append('path').attr('d', function (d) {
          return linegen(d.filter(function (d) {
            return d.values[0][lineYSel] !== null;
          }));
    ...
    processedLineData = lineData.map(function (d) {
    ...
    var nestedLineData = _$d3Node_32.nest().key(function (d) {
      return d.age;
    }) // this is what we'll group by
